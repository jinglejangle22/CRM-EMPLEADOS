...
export function CompanyStatGrid({
  ...
  return (
    <section className="rounded-2xl bg-white p-4 ring-1 ring-neutral-100">
      <h2 className="text-sm font-semibold text-neutral-900">Personal activo</h2>
      <p className="mt-1 text-3xl font-semibold text-neutral-900">{total}</p>
      <div className="mt-4 grid grid-cols-3 gap-2">
        {allCompanies.map((c) => (
          <div key={c.id} className="rounded-xl bg-neutral-50 p-2.5 text-center">
            ...
            <p className="text-[11px] leading-tight text-neutral-500">{c.shortName}</p>
          </div>
        ))}
      </div>
    </section>
